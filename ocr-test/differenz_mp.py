...
def calculate_difference(path1, path2):
    ...
    if file_a != False and file_b != False:
        ...
        return (idstring, length, lev_norm)


def calculate_difference_mp(path_tuple):
    print "Ermittle Differenz für", path_tuple[0], "und", path_tuple[1]
    return calculate_difference(path_tuple[0], path_tuple[1])


def single_result_display(idstring, length, levenshtein_distance):
    print "%s: %d von %d Zeichen - %.1f %%" % (
        idstring,
...
if __name__ == '__main__':
    ...
    if os.path.isfile(args[0]) and os.path.isfile(args[1]):
        (idstring, length, difference) = calculate_difference(args[0], args[1])
        single_result_display(idstring, length, difference)
        ...
        for filename in files:
            ...
            joblist.append((path1, path2))
        pool = Pool(processes=4)
        results = pool.map(calculate_difference_mp, joblist)
        stats = []
        print "--------------------------------------------------------"
        for (idstring, length, distance) in results:
            stats.append(float(distance) / float(length))
            single_result_display(idstring, length, distance)
        (avg, std) = average_stdev(stats)
        print "--------------------------------------------------------"
